...
public class VariableIncrementGetNode extends Node {
    private final String variableName;

    public VariableIncrementGetNode(ExecutionContext context, String variableName) {
        super(context);
    ...
    public OutputResult execute() throws ExecuteException {
        ...
        else throw new ExecuteException(new StyledText("Cannot increment " + c.getSimpleName()));
        OutputResult outputResult = new OutputResult(context, result, c, Utils.texts(
                context.getDisplayFormat().text(FormatType.OPERATOR, "++"),
                context.getDisplayFormat().text(FormatType.VARNAME, variableName)
        ));
        context.getVariables().put(variableName, outputResult);
        return outputResult;
    }
}